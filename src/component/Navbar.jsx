...
function Navbar() {
  ...
  return (
    <>
      <div className='max-w-screen-2xl container mx-auto md:px-20 h-16 shadow-md fixed top-0 left-0 rigth-0 bg-white'>
        <div className='flex justify-between items-center h-16'>
          <div className='flex space-x-2'>
            <img src={pic} className='h-12 w-12 rounded-full' />
            <h1 className='font-semibold text-xl curser-pointer'>
              Muska<span className='text-green-500 text-2xl'>n</span>
              <p>Web Developer</p>
            </h1>
          </div>
                  ...
                  navitems.map(({ id, text }) => (
                    <li className='hover:scale-105 duration-200 cursor-pointer font-semibold' key={id}>
                      <Link onClick={() => setmenu(!menu)}
                        ...
                        smooth={true}
                        duration={500}
                        offset={-70} activeClass='active'>{text}</Link>
                    </li>
                  ))
                }
              </ul>
